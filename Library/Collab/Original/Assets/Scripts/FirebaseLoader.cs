...
namespace Microsoft.Azure.SpatialAnchors.Unity.Examples
{
    public class FirebaseLoader : FirebaseManager
    {
        ...
        private void AddLeanTouch(GameObject obj)
        {
            LeanSelectable selectable = obj.AddComponent<LeanSelectable>();

            obj.AddComponent<LeanPinchScale>().Use.RequiredSelectable = selectable;
            obj.AddComponent<LeanDragTranslate>().Use.RequiredSelectable = selectable;
            obj.AddComponent<LeanTwistRotate>().Use.RequiredSelectable = selectable;
        }
        ...
        public void CleanupSpawnedModels()
        {
            foreach (var entry in spawnedObjects)
            {
                if (entry.Value != null && entry.Value.obj != null)
                    entry.Value.obj.SetActive(false);
            }
